...
Crawl Module (external extension)
   → Fetch HTML / API from live e-commerce sites
   → Parse & normalize
   → Save to JSON/CSV
...
│   ├── staging/                 # Intermediate (ignored)
│   └── processed/               # Analytics-ready exports (ignored)
│
├── crawl/                       # Web crawling module (external)
│   ├── core/                    # Fetching, parsing, storage
│   ├── sites/                   # Site adapters (Amazon demo, Taobao, JD, ...)
│   ├── configs/                 # Selector/API configs
│   ├── pipelines/               # Data normalization & cleaning
│   ├── jobs/                    # Crawl entry points
│   └── tests/                   # Unit tests
│
├── src/                         # Python ETL source
├── jobs/                        # ETL jobs (sample gen, ingestion)
...
- ✔️ dbt staging models built (orders, customers, products, items, payments, reviews)
- ✔️ dbt marts created (fact_orders, dim_customers, dim_products)
- ✔️ Custom macros fixed → clean `staging` / `analytics` schemas
- ✔️ dbt lineage docs generated
- ✔️ Crawl module added → Amazon demo working

## 📈 Next Steps

- [ ] Implement `pipelines/normalize.py` for crawl data (price, review count cleaning)
- [ ] Add more site adapters (Taobao, JD)
- [ ] Store crawled data into Postgres (raw schema)
- [ ] Integrate crawlers into Airflow for scheduling
- [ ] Build BI dashboards (daily sales, top products, payment methods)
